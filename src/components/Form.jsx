...
function TextInput({ type = 'text', label }) {
    ...
    return (
      <div className="input-container mb-4">
        ...
        <label className={`${value && 'filled'} d-flex align-items-center gap-2`} htmlFor={name}>
          {label}
        </label>
      </div>
    );
            ...
            <section className="d-flex justify-content-center mb-4 mx-auto rounded">
                    <form action="https://api.web3forms.com/submit" method="POST" className="text-center" style={{ width: '100%', maxWidth: '500px' }}>
                        ...
                        <XyzTransition appearVisible xyz="fade-100% down-50% duration-15">
                          <div>
                            <TextInput label={[<AiOutlineUser style={{color: "#BAFF29", backgroundColor: "transparent", width: "15px"}} />, " Nombre"]} />
                          </div>
                        </XyzTransition>
                        <XyzTransition appearVisible xyz="fade-100% down-50% duration-15 delay-5">
                          <div>
                            <EmailInput label={[<AiOutlineMail style={{color: "#BAFF29", backgroundColor: "transparent", width: "15px"}} />, " Correo"]} />
                          </div>
                        </XyzTransition>
                        <XyzTransition appearVisible xyz="fade-100% down-50% duration-15 delay-5">
                          <div>
                            <TextAreaInput label={[<AiOutlineMessage style={{color: "#BAFF29", backgroundColor: "transparent", width: "15px"}} />, " Mensaje"]} />
                          </div>
                        </XyzTransition>
                        <XyzTransition appearVisible xyz="fade-100% down-50% duration-15 delay-5">
                          <div>
                            <button button className="w-100 fw-bold btn btn-block btn-lg mb-0" type="submit" value="Submit" style={{ backgroundColor: '#BAFF29'}}>Enviar</button>
                          </div>
                        </XyzTransition>
                    </form>
                </section>
